Y.use("node", "squarespace-gallery-ng", "squarespace-image-loader", function (a) {
    window.Site = Singleton.create({
        ...
        setSidebarFixed: function () {
            a.one(".sidebar-fixed") && (a.one("body").get("winWidth") this.getBodyWidth()) {
        ...
        if (b) var b = b.replace(/\#itemId\=/, ""),
            c = a.one('[data-slide-id\x3d"' + b + '"]'),
            d = setInterval(function () {
                c.one("img").hasClass("loaded") && (setTimeout(function () {
                    window.scrollTo(0, c.getY())
                }, 200), clearInterval(d))
            }, 100)
    } else a.one("body.enable-gallery-thumbnails.initial-gallery-view-thumbnails") || a.one("body.enable-gallery-thumbnails.homepage.homepage-gallery-view-thumbnails") ? a.one("body.enable-gallery-thumbnails.homepage.homepage-gallery-view-slideshow") ? this.setupSlideshow() : (b = window.location.hash, this.setupSlideshow(), b ? a.later(100, this, function () {
        ...
        this.checkHeaderHeight()
    }) : this.showThumbnails()) : this.setupSlideshow(), a.all(".thumbnail-toggle").each(function (a) {
        a.on("click", function () {
            this.showThumbnails()
        }, this)
    }, this), a.one("#thumbnails").delegate("click", function (b) {
    ...
    setupThumbnailGrid: function () {
        this.thumbnails || (this.thumbnails = new a.Squarespace.Gallery2({
    ...
    setupSlideshow: function (b) {
        if (!this.slideshow) {
            a.Squarespace.Template.getTweakValue("galleryPadding") && a.one("#slideshowWrapper").setStyle("height", parseInt(a.one("body").getComputedStyle("height"), 10) - 2 * parseInt(a.Squarespace.Template.getTweakValue("galleryPadding"), 10) + "px");
            if (a.one("#slideshow .slide")) {
                var c = "fit";
                a.Squarespace.Template.getTweakValue("gallery-style") && (c = a.Squarespace.Template.getTweakValue("gallery-style").toLowerCase());
